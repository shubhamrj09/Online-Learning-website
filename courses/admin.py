...
class UserCourseAdminModel(admin.ModelAdmin):
    ...
    list_filter = ['course']
    enrolled_date = model.DateTimeField(auto_now_add=True)
...
class UserCourseAdminModel(admin.ModelAdmin):
    model = UserCourse
    list_display = ['click', 'get_user', 'get_course', 'enrolled_date','get_completion_status']
    list_filter = ['course', ('enrolled_date', DateFieldListFilter),'is_completed']  # ✅ Filter by date
    ...
    def enrolled_date(self, usercourse):
        return usercourse.enrolled_date.strftime("%Y-%m-%d %H:%M")
    
    def get_completion_status(self, usercourse):
        return 'Completed' if usercourse.is_completed else 'In Progress'
    get_completion_status.short_description = 'Completion Status'

    enrolled_date.admin_order_field = 'enrolled_date'
    enrolled_date.short_description = 'Enrolled Date'
